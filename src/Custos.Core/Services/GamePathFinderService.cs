...
public class GamePathFinderService
{
    ...
    /// <summary>
    /// Finds all possible Unturned installation paths across all Steam libraries.
    /// Returns multiple paths for thorough scanning.
    /// </summary>
    public List<string> FindUnturnedPaths()
    {
        var paths = new List<string>();

        // Get from Steam libraries
        var libraries = GetSteamLibraries();
        foreach (var library in libraries)
        {
            var unturnedPath = Path.Combine(library, "common", "Unturned");
            if (Directory.Exists(unturnedPath))
                paths.Add(unturnedPath);
        }

        // Try cached path
        var cachedPath = GetUnturnedPath();
        if (!string.IsNullOrEmpty(cachedPath) && !paths.Contains(cachedPath))
